...
const ajax = (obj) => {
  ...
  if (window.XMLHttpRequest) {
    ...
    // ie
    ajaxs = new window.ActiveXObject('Microsoft.XMLHTTP');
  }
  // 区分get和post
  ...
  ajaxs.onreadystatechange = () => {
    if (+ajaxs.readyState === 4) {
      if ((ajaxs.status >= 200 && ajaxs.status < 300) || (ajaxs.status === 304)) {
        obj.success(JSON.parse(ajaxs.responseText));
      } else {
        obj.error(JSON.parse(ajaxs.status));
      }
    }
  };
};
