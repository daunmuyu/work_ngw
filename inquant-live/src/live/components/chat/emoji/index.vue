<template>
	<div class="chatemoji clearfix">
			<div class="imgemoji f-l" v-for="item in 68" :key="item" :style="'background-position:0 '+(item-1)*(-1.5555555555555556)+'rem;'" @click.stop="clickemoji(item)">
			</div>
	</div>
</template>
...
export default {
	...
	methods: {
		clickemoji(index){
      const curremojicode = this.emojiCode[index-1];
      this.$emit('emojiSelected', `[${curremojicode}]`);
			
			// this.$store.commit('setEmojiShow',false);
			// this.$store.commit('addEmoji',{ifadd: true,cont: `[${curremojicode}]`,});
		},
	},
}
</script>
...
.chatemoji{
     ...
     left: 0rem;
     bottom: 29px;
     padding: 0.9444444444444444rem;
     border-radius: 0.5rem;
     width: 16.11111111111111rem;
     z-index: 101;
     background: #fff;
     ...
     & .imgemoji{
         ...
         background-image: url(../../../../images/emojiimg.png);
         background-repeat: no-repeat;
         background-size:1.5555555555555556rem 105.77777777777777rem;
         cursor: pointer;
         float: left;
    }
}
