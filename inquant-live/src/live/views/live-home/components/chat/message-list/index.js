...
export default Vue.extend({
  ...
  mounted() {
    ...
    if (this.isDayMode === 'day') {
      this.pic = 'https://h5.inquant.cn/future/inquant-img/pcday.png'
    } else {
      this.pic = 'https://h5.inquant.cn/future/inquant-img/pcnight.png'
    }
  },
  watch: {
    list(nList, oList) {
  ...
  methods: {
    ...
    isToday(timeStr) {
      const now = format(new Date(new Date().getTime()), 'yyyy-MM-dd hh:mm:ss');
      const old = format(new Date(timeStr * 1000), 'yyyy-MM-dd hh:mm:ss')
      console.log(now.substr(0, 10))
      return now.substr(0, 10) === old.substr(0, 10);
    },
